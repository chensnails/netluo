...
#   sudo bash install.sh --drawio http://192.168.1.20:3091 --password 'xxx'
#
...
[ -n "$DRAWIO_URL" ] || DRAWIO_URL="http://${HOST_IP:-127.0.0.1}:3091"
...
EOF
[ "${DRAWIO_URL#http://127.0.0.1}" = "$DRAWIO_URL" ] || \
  warn "DRAWIO_URL 指向 127.0.0.1，别人浏览器里打不开画布；确认后用 install.sh --drawio http://<对外地址>:3091 重跑"
